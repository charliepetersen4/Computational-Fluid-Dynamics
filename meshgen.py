...
def writeGeoFromXYZVarySizing(a_XYZFile, Inputs, a_ParametricSize=True):
    ...
    with open(geofile,"w+") as geoFile:
        ...
        Points = f"Point(1000) = {{{xmin}, {ymin}, 0, {Chord}}}; " \
             f"Point(1001) = {{{xmax}, {ymin}, 0, {Chord}}}; " \
             f"Point(1002) = {{{xmax}, {ymax}, 0, {Chord}}}; " \
             f"Point(1003) = {{{xmin}, {ymax}, 0, {Chord}}};\n"
        
        Lines = """Line(101) = {1000,1001}; 
                Line(102) = {1001,1002}; 
                Line(103) = {1002,1003}; 
                Line(104) = {1003,1000}; """
    ...
    elemTypes, elemTags, elemNodeTags = gmsh.model.mesh.getElements()
    num_elements = sum(len(tags) for tags in elemTags)
    ...
    print(f"  {geofile}")
    print(f"  {meshfile}\n")
    print(f"Mesh contains {num_nodes} nodes and {num_elements} elements.")
