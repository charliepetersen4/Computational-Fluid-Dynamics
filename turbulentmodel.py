...
def checkLESMeshResolution(a_Mesh, a_Viscosity, a_Density, a_U0, a_Diam, histFile='mesh-les-check.png'):
    ...
    if a_Mesh.comm.rank == 0:
        ...
        eps_est = a_U0**3 / L_int       # dissipation estimate   [m²/s³]
        ...
        axes[0].set_xlabel('Cell diameter (h) [m]', fontweight='bold')
...
meshFile = 'geom-vonkarman-circle-Re1000.msh'   # for LES
...
if useLES:
    mesh_ok = checkLESMeshResolution(mesh, viscosity, density, U0, diam, histFile='mesh-check.png')
    if not mesh_ok:
